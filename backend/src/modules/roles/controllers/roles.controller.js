...
const updatePermissions   = _h(async (req, reply)  => { reply.send(await RolesService.updateRolePermissions(req.params.id, req.body.permissions)); });
const deleteRole          = _h(async (req, reply)  => {
  await RolesService.deleteRole(req.params.id);
  reply.code(httpStatus.NO_CONTENT).send(null);
});

const RolesController = { getRoles, createRole, updateRole, updatePermissions, deleteRole };
