...
export const getMyProgress = _handle(async (req, reply) => {
  const { dateFrom, dateTo } = req.query;
  reply.send(await svc.getMyProgress(req.user.id, { dateFrom, dateTo }));
});
...
export const getCalendar = _handle(async (req, reply) => {
  ...
  const { dateFrom, dateTo } = req.query;
  reply.send(await svc.getCalendar(req.user.id, dateParam, { dateFrom, dateTo }));
});
