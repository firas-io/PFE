    ...
    <DefaultLayout
      ...
      beforeFields={
        <div className="d-grid gap-3 sm:grid-cols-2">
          <div className="d-flex flex-column align-items-center justify-content-center rounded-4 border border-emerald-500/25 bg-emerald-500/5 py-4">
            <span className="text-[10px] fw-semibold text-uppercase tracking-wide text-muted">
              Humeur
            </span>
            <motion.div
              key={humeurKey}
              ...
              className="mt-2 text-5xl lh-1"
              aria-label={`Humeur ${humeurKey}`}
            >
              {MOOD[humeurKey] ?? "😐"}
            </motion.div>
          </div>
          <div className="min-w-0 space-y-1 rounded-4 border border-border/50 bg-muted/10 p-2">
