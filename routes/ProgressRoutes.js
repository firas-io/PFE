...
// Helper function to determine if a habit should be shown on a specific date
function isHabitScheduledForDate(habit, targetDate) {
  ...
  // If specific dates are set, check if today is in the list
  ...
  // If no specific dates set, use frequency mode
  // For now, show all habits by default (daily frequency)
  console.log(`[SCHEDULE] Habit "${habit.nom}" (no specific dates, frequence=${frequence}): SHOW`);
  return true;
}

async function routes(fastify) {
  fastify.get("/progress/my", { preHandler: [fastify.authenticate] }, async (req, reply) => {
    try {
  ...
  // Nouvelle route pour récupérer les logs du jour
  fastify.get("/progress/today", { preHandler: [fastify.authenticate] }, async (req, reply) => {
    try {
      const habits = await Habit.find({
        $or: [{ utilisateur_id: req.user.id }, { visible_pour_tous: true }]
  ...
  fastify.get("/progress/calendar", { preHandler: [fastify.authenticate] }, async (req, reply) => {
    try {
      ...
      // Fetch FULL habit details (not just fields) to get frequency info
      const habits = await Habit.find({
        $or: [{ utilisateur_id: req.user.id }, { visible_pour_tous: true }]
      });
      ...
      const habitsForSelectedDay = habits.filter((h) => {
        const shouldShow = isHabitScheduledForDate(h, requestedDate);
        return shouldShow;
      });
