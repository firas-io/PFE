...
async function routes(fastify) {
  ...
  // POST /logs — Propriétaire de l'habitude, admin, ou habitude visible pour tous
  fastify.post("/logs", {
  ...
  // GET /logs — RÉSERVÉ AUX ADMINS (Permission: LOGS_VIEW)
  fastify.get("/logs", {
    preHandler: [fastify.authenticate, fastify.authorize(["LOGS_VIEW"])]
  }, async (req, reply) => {
    try {
  ...
  // GET /logs/:id — Propriétaire ou Admin (Permission: LOGS_VIEW)
  fastify.get("/logs/:id", {
    preHandler: [fastify.authenticate]
  }, async (req, reply) => {
    try {
  ...
  // PUT /logs/:id — Propriétaire ou Admin (Permission: LOGS_MANAGE)
  fastify.put("/logs/:id", {
    preHandler: [fastify.authenticate]
  }, async (req, reply) => {
    try {
  ...
  // DELETE /logs/:id — Propriétaire ou Admin (Permission: LOGS_MANAGE)
  ...
  fastify.get("/logs/incomplete-for-date/:date", {
    preHandler: [fastify.authenticate]
  }, async (req, reply) => {
    try {
  ...
  // POST /logs/catchup — Submit a catch-up with photo verification
  fastify.post("/logs/catchup", {
    preHandler: [fastify.authenticate]
  }, async (req, reply) => {
    try {
